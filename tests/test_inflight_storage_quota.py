...
def _inflight_source_block() -> str:
    start = UI_JS.index("const INFLIGHT_KEY = 'hermes-webui-inflight';")
    end = UI_JS.index("function showReconnectBanner", start)
...
def test_inflight_state_limits_are_configurable_from_settings():
    ...
    assert "function _inflightStateLimits()" not in UI_JS, (
        "Function name must not collide with window._inflightStateLimits config object (#2771)."
    )
...
def test_mark_inflight_recovers_when_storage_is_full(tmp_path):
    """Executing markInflight should retry after clearing bulky recovery snapshots."""
