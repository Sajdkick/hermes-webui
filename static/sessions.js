...
async function loadSession(sid){
  ...
  if (resetConversationPane) {
    // #3306: When force-reloading the currently-active session, snapshot
    // existing messages before clearing so ephemeral turn fields survive the
    // authoritative reload.
    _pendingCarryForwardSnapshot = (currentSid === sid && forceReload)
      ? (S.messages || []).slice()
      : null;
    // #3239: capture a reload-width hint before clearing so the reload keeps
    // the already-loaded transcript width.
    ...
    if (_msgInner && (currentSid !== sid || needsRecovery)) _msgInner.innerHTML = '<div style="display:flex;align-items:center;justify-content:center;height:100%;color:var(--text-muted);font-size:14px;padding:40px;text-align:center;">'+(currentSid===sid?'Refreshing conversation…':'Loading conversation...')+'</div>';
  }
  ...
  try {
    ...
    if(_msgInner){
      if(e.status===404){
        ...
        // (!currentSid) or a mid-session reload of the current session whose
        // sidecar was deleted server-side. A click into a different dead
        // session must not wipe the healthy current session.
        if(!currentSid || currentSid===sid){
...
async function _ensureMessagesLoaded(sid, options) {
  ...
  let data;
  try {
    data = await api(`/api/session?session_id=${encodeURIComponent(sid)}&messages=1&resolve_model=0${reloadLimitParam}${lineageParam}`);
  } finally {
    _clearSameSessionForceReloadHint(sid);
  }
...
async function _loadOlderMessages() {
  ...
  try {
    ...
    const data = await api(`/api/session?session_id=${encodeURIComponent(sid)}&messages=1&resolve_model=0&msg_limit=${requestedLimit}${lineageParam}`);
    // Guard: api() may have redirected (401) and returned undefined.
    ...
    if (!tailMatches) {
      ...
      const fallback = await api(`/api/session?session_id=${encodeURIComponent(sid)}&messages=1&resolve_model=0&msg_before=${_oldestIdx}&msg_limit=${_INITIAL_MSG_LIMIT}${lineageParam}`);
...
async function deleteSession(sid, beforeDelete=null){
  ...
  const session=_sessionSnapshotById(sessionId);
  const ok=await showConfirmDialog({
  ...
  const deleteRequest=api('/api/session/delete',{method:'POST',body:JSON.stringify({session_id:sessionId})}).then(response=>{
    _clearHandoffStorageForSession(sessionId);
    return {response};
  }, error=>({error}));
  if(beforeDeleteHold){
    await beforeDeleteHold;
    _optimisticallyRemovedSessionIds.add(sessionId);
    _pendingSessionReflowPositions=reflowPositions;
    _optimisticallyRemoveSessionFromList(sessionId);
  ...
  if(deleteResult&&deleteResult.error){
    ...
    if(optimisticRendered){
      _optimisticallyRemovedSessionIds.delete(sessionId);
  ...
  if(optimisticRendered) void renderSessionList().finally(()=>_optimisticallyRemovedSessionIds.delete(sessionId));
