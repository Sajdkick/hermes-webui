...
const LOCALES = {
  ...
  folder_add_as_space_title: 'Add as Space?',
    ...
    workspace_uploading: (n, dir) => `Uploading ${n} file(s) to ${dir}\u2026`,
    workspace_upload_preparing: (files, dirs, dir) => `Preparing ${files} file(s)${dirs ? ` and ${dirs} folder(s)` : ''} for ${dir}`,
    workspace_upload_creating_dirs: (current, total) => `Creating folder ${current} of ${total}\u2026`,
    workspace_upload_progress: (current, total, name, percent, bytes) => `File ${current} of ${total}: ${name} (${percent}%, ${bytes})`,
    workspace_upload_indicator_title: 'Uploading to workspace\u2026',
    workspace_upload_complete: (uploaded, createdDirs, dir) => `Added ${uploaded} file(s)${createdDirs ? ` and ${createdDirs} folder(s)` : ''} to ${dir}`,
    workspace_upload_partial: (uploaded, createdDirs, failed) => `Workspace upload complete: ${uploaded} file(s) and ${createdDirs} folder(s) added, ${failed} failed`,
    workspace_upload_no_files: 'No files were found in that drop.',
    uploading: 'Uploading…',
    uploaded: 'Uploaded',
    ...
    workspace_paths_validated_hint: 'Existing directories are accepted; missing folders are created when you save.',
  ...
  folder_add_as_space_title: 'スペースとして追加?',
    ...
    workspace_upload_indicator_title: 'ワークスペースへアップロード中…',
    workspace_upload_preparing: (files, dirs, dir) => `${files} 件のファイル${dirs ? `と ${dirs} 件のフォルダ` : ''}を ${dir} 用に準備中`,
    workspace_upload_creating_dirs: (current, total) => `フォルダ ${current}/${total} を作成中…`,
    workspace_uploading: (n, dir) => `${n} 件のファイルを ${dir} へアップロード中…`,
    workspace_upload_progress: (current, total, name, percent, bytes) => `ファイル ${current}/${total}: ${name} (${percent}%, ${bytes})`,
    workspace_upload_complete: (uploaded, createdDirs, dir) => `${uploaded} 件のファイル${createdDirs ? `と ${createdDirs} 件のフォルダ` : ''}を ${dir} に追加しました`,
    workspace_upload_partial: (uploaded, createdDirs, failed) => `ワークスペースのアップロード完了: ${uploaded} 件のファイルと ${createdDirs} 件のフォルダを追加、${failed} 件失敗`,
    workspace_upload_no_files: 'ドロップされた内容にファイルが見つかりませんでした。',
  ...
  folder_add_as_space_title: 'Добавить как пространство?',
    ...
    workspace_uploading: (n, dir) => `Загрузка ${n} файл(ов) в ${dir}…`,
    workspace_upload_preparing: (files, dirs, dir) => `Подготовка ${files} файл(ов)${dirs ? ` и ${dirs} папок` : ''} для ${dir}`,
    workspace_upload_progress: (current, total, name, percent, bytes) => `Файл ${current} из ${total}: ${name} (${percent}%, ${bytes})`,
    workspace_upload_complete: (uploaded, createdDirs, dir) => `Добавлено ${uploaded} файл(ов)${createdDirs ? ` и ${createdDirs} папок` : ''} в ${dir}`,
    workspace_upload_partial: (uploaded, createdDirs, failed) => `Загрузка в рабочее пространство завершена: добавлено ${uploaded} файл(ов) и ${createdDirs} папок, ошибок: ${failed}`,
    workspace_upload_no_files: 'В этом перетаскивании не найдено файлов.',
    workspace_upload_creating_dirs: (current, total) => `Создание папки ${current} из ${total}…`,
    workspace_upload_indicator_title: 'Загрузка в рабочее пространство…',
  ...
  folder_add_as_space_title: '¿Añadir como espacio?',
    ...
    workspace_uploading: (n, dir) => `Subiendo ${n} archivo(s) a ${dir}…`,
    workspace_upload_preparing: (files, dirs, dir) => `Preparando ${files} archivo(s)${dirs ? ` y ${dirs} carpeta(s)` : ''} para ${dir}`,
    workspace_upload_progress: (current, total, name, percent, bytes) => `Archivo ${current} de ${total}: ${name} (${percent}%, ${bytes})`,
    workspace_upload_complete: (uploaded, createdDirs, dir) => `Añadidos ${uploaded} archivo(s)${createdDirs ? ` y ${createdDirs} carpeta(s)` : ''} a ${dir}`,
    workspace_upload_partial: (uploaded, createdDirs, failed) => `Subida al espacio completada: ${uploaded} archivo(s) y ${createdDirs} carpeta(s) añadidos, ${failed} fallidos`,
    workspace_upload_no_files: 'No se encontraron archivos en ese arrastre.',
    workspace_upload_creating_dirs: (current, total) => `Creando carpeta ${current} de ${total}…`,
    workspace_upload_indicator_title: 'Subiendo al espacio de trabajo…',
  ...
  folder_add_as_space_title: '添加为工作区？',
    ...
    workspace_uploading: (n, dir) => `正在将 ${n} 个文件上传到 ${dir}…`,
    workspace_upload_preparing: (files, dirs, dir) => `正在准备 ${files} 个文件${dirs ? ` 和 ${dirs} 个文件夹` : ''} 到 ${dir}`,
    workspace_upload_progress: (current, total, name, percent, bytes) => `第 ${current}/${total} 个文件：${name}（${percent}%，${bytes}）`,
    workspace_upload_complete: (uploaded, createdDirs, dir) => `已将 ${uploaded} 个文件${createdDirs ? ` 和 ${createdDirs} 个文件夹` : ''}添加到 ${dir}`,
    workspace_upload_partial: (uploaded, createdDirs, failed) => `工作区上传完成：已添加 ${uploaded} 个文件和 ${createdDirs} 个文件夹，${failed} 个失败`,
    workspace_upload_no_files: '此次拖放中未找到可上传的文件。',
    workspace_upload_creating_dirs: (current, total) => `正在创建第 ${current}/${total} 个文件夹…`,
    workspace_upload_indicator_title: '正在上传到工作区…',
    // settings panel
    settings_title: '设置',
  ...
  ko: {
    ...
    uploaded: 'Uploaded',
    all_uploads_failed: (n) => `All ${n} upload(s) failed`,
    workspace_uploading: (n, dir) => `${dir}에 ${n}개 파일 업로드 중…`,
    workspace_upload_preparing: (files, dirs, dir) => `${dir}에 보낼 ${files}개 파일${dirs ? ` 및 ${dirs}개 폴더` : ''} 준비 중`,
    workspace_upload_progress: (current, total, name, percent, bytes) => `파일 ${current}/${total}: ${name} (${percent}%, ${bytes})`,
    workspace_upload_complete: (uploaded, createdDirs, dir) => `${dir}에 ${uploaded}개 파일${createdDirs ? ` 및 ${createdDirs}개 폴더` : ''} 추가됨`,
    workspace_upload_partial: (uploaded, createdDirs, failed) => `워크스페이스 업로드 완료: ${uploaded}개 파일 및 ${createdDirs}개 폴더 추가, ${failed}개 실패`,
    workspace_upload_no_files: '드롭한 항목에서 파일을 찾지 못했습니다.',
    workspace_upload_creating_dirs: (current, total) => `폴더 ${current}/${total} 생성 중…`,
    workspace_upload_indicator_title: '워크스페이스에 업로드 중…',
    session_pin: 'Pin conversation',
    session_unpin: 'Unpin conversation',
    session_pin_desc: 'Keep this conversation at the top',
    session_unpin_desc: 'Remove from pinned',
