...
from api.upload import (
    ...
    handle_transcribe,
    handle_workspace_generic_upload,
...
def handle_get(handler, parsed) -> bool:
    ...
    if parsed.path == "/api/chat/stream/status":
        ...
        payload = {"active": active, "stream_id": stream_id, "replay_available": False, "partial_text": partial_text, "reasoning_text": reasoning_text, "tool_calls": live_tool_calls}
        try:
...
def start_project_git_conflict_analysis_turn(session_id: str, prompt: str, metadata: dict | None = None) -> dict:
    """Start the first analysis-only turn for a project Git conflict handoff."""
    try:
        s = get_session(session_id)
    except KeyError:
        return {"ok": False, "error": "Session not found"}
    ...
    status = int(response.pop("_status", 200) or 200)
    if status >= 400:
        return {"ok": False, "error": response.get("error") or "Could not start conflict analysis turn", **response}
    return {"ok": True, **response}
