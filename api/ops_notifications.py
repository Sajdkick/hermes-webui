...
def _play_notification(project: dict, status: dict) -> dict | None:
    ...
    payload = {
        ...
        "inspectMode": _text(status.get("inspectMode") or status.get("inspect_mode"), limit=64),
        "allocatedPort": status.get("allocatedPort") or status.get("allocated_port"),
        "allocatedPortHost": _text(status.get("allocatedPortHost") or status.get("allocated_port_host"), limit=256),
