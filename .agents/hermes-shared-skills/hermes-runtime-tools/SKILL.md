...
- Treat managed screenshots as best-effort. If runtime screenshot capture is not available or times out after Play has reported ready, use a direct served-artifact verification path before giving up: read the allocated host/port from `hermes-runtime play status --json`, fetch the app route (commonly `/app`) and expect HTTP 200 HTML, parse the index chunk for lazy route chunks, then fetch the relevant chunk and assert the expected user-facing strings or module markers are present. Report the screenshot limitation separately from app readiness.
...
- `references/live-lazy-chunk-verification.md` — direct HTTP/lazy-bundle verification pattern for proving a ready Play server is serving a specific UI change when screenshot capture is unavailable or times out.
