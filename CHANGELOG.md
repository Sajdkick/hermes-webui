...
- Ops task launch is now idempotent for existing linked task sessions: relaunching a task reuses the current non-archived Ops task session instead of creating a duplicate session with the same task, and the dashboard opens the reused session instead of sending the task prompt again. The active sessions section also collapses historic duplicate Ops task sessions for the same project/task down to the current visible session.
- Ops dashboard active-session close now archives the explicit visible row/session lineage instead of trusting a stale task linkage that may resolve to an already-archived continuation, so duplicate Summons/Ops task rows actually disappear after confirmation.
- Play builds can now auto-detect a package-script workflow when a project has `package.json` scripts but no `project_play.json`/`.cloud-terminal/play.json`, so the Ops Build control is not blocked solely by a missing Play config file.
- Ops Play/Build buttons now use an explicit build-capability status (`buildAvailable`/`canBuild`) instead of treating `configExists` as the source of truth, so package-script and shared Play workflows stay buildable without a legacy per-project config file and initial project views no longer report a missing Play config before status hydration completes.
- Ops dashboard **Execute all tasks** and **Create & Run** sessions using OpenAI Codex OAuth now prime runtime-visible Codex tokens against the task session's assigned profile home instead of the process/default active profile, and repair stale named-profile Codex tokens from the root profile when the profile-local refresh token has been consumed. Project-scoped task sessions no longer fail with `OPENAI-CODEX_API_KEY` despite a successful browser OAuth login in the root profile.
- Play builds now have a bounded build-stage timeout (`build.timeoutMs`, default 30 minutes) so a hung build cannot leave the Ops dashboard stuck on a locked build notification for hours; timed-out builds terminate the process group and surface a failed Play notification with the timeout reason.
- Ops dashboard session opens now enter a simplified chat-inspection mode that hides the rail/sidebar/workspace chrome and shows a single “Back to Ops dashboard” control while keeping the composer available for follow-up messages; standalone `/ops` redirects persist the same inspect mode for the target session.
- Ops entry points opened from `/session/<id>` pages now navigate to the site-root `/ops-phase` shell instead of resolving a session-relative `/session/<id>/ops` URL, and stale `/session/<id>/ops` or `/session/<id>/ops-phase` requests now redirect server-side to the canonical shell as a fallback. This fixes the unstyled white legacy page shown from cached mobile or embedded session flows.
- Ops dashboard task execution now selects the project’s assigned Hermes profile before starting the task turn, so project-scoped task sessions run under the same profile they were created with instead of falling back to the previously active profile.
